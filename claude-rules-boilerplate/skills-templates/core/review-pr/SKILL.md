...
Senior-level holistic review with a 40-point rubric. This is the standalone version of Phase 6 from the feature-lifecycle. The Tech Lead reviews the consolidated PR diff for cross-file consistency and overall quality.

## Triggers

- `/review-pr` -- review current branch against main
- `/review-pr NNN` -- review PR #NNN
- `/review-pr STORY-ID` -- review by story ID

## Prerequisites

- Code must be committed
- Branch should have changes relative to main
- Ideally, specialist reviews (`/review`) have already been run
